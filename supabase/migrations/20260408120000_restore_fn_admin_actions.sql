-- 🚀 [BanjaraBio] Restore full fn_admin_actions stripped by 20260408110000
-- Purpose: Recovers get_admin_stats full logic and missing branches.

DROP FUNCTION IF EXISTS public.fn_admin_actions(text, jsonb);
CREATE OR REPLACE FUNCTION public.fn_admin_actions(action TEXT, p_payload JSONB DEFAULT '{}'::jsonb) RETURNS JSONB AS $$
DECLARE 
    v_admin_id UUID := auth.uid();
    v_search_query TEXT := COALESCE(p_payload->>'search_query', '');
    v_result JSONB;
...
VALUES (
        upper(trim((p_payload->>'code')::TEXT)),
        (p_payload->>'offer_name')::TEXT,
        (p_payload->>'description')::TEXT,
        (p_payload->>'valid_until')::DATE,
        COALESCE((p_payload->>'discount_percentage')::INTEGER, 0)
    )
RETURNING jsonb_build_object('id', id) INTO v_result;
WHEN 'update_coupon' THEN
UPDATE public.coupons
SET offer_name = COALESCE((p_payload->>'offer_name')::TEXT, offer_name),
    ...
    discount_percentage = COALESCE(
        (p_payload->>'discount_percentage')::INTEGER,
        discount_percentage
    ),
    is_active = COALESCE((p_payload->>'is_active')::BOOLEAN, is_active),
    updated_at = NOW()
