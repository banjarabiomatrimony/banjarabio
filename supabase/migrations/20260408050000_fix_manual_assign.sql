...
DECLARE
    ...
    v_role TEXT;
    v_assigned_count INT := 0;  -- Newly declared to safely capture ROW_COUNT and iterate
BEGIN
    -- 1. 🛡️ SECURITY CHECK
    ...
    CASE action
        ...
        WHEN 'manual_assign' THEN
            ...
            IF v_assigned_count > 0 THEN
                PERFORM public.fn_trigger_push_notification(v_staff_id, 'New Leads Assigned 🎯', 'Admin assigned ' || v_assigned_count || ' new leads to you.', '{"route":"/staff-dashboard"}'::jsonb);
            END IF;
        ...
        WHEN 'auto_assign_leads' THEN
            DECLARE
                ...
                v_num_staff INT;
            BEGIN
                -- v_assigned_count already mapped to 0 globally, but resetting here for safety
                v_assigned_count := 0; 
