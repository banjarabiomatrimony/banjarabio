-- 📜 [BanjaraBio] ADMIN & TELECALLER NOTIFICATIONS
-- Purpose: Trigger push notifications for lead assignments and conversions
-- Version: 1.0
-- Date: 2026-04-08

-- 1. 🔄 UPDATE `fn_admin_team` TO INCLUDE PUSH NOTIFICATIONS
...
BEGIN
    ...
    CASE action
        ...
        -- 📎 [MANUAL ASSIGN] Granular bulk assignment (WITH NOTIFICATION)
        WHEN 'manual_assign' THEN
            ...
            GET DIAGNOSTICS v_target_user_id = ROW_COUNT;
            
            -- PUSH NOTIFICATION 🔔
            IF v_target_user_id > 0 THEN
                PERFORM public.fn_trigger_push_notification(v_staff_id, 'New Leads Assigned 🎯', 'Admin assigned ' || v_target_user_id || ' new leads to you.', '{"route":"/staff-dashboard"}'::jsonb);
            END IF;
            
            v_result := jsonb_build_object('status', 'success', 'assigned', v_target_user_id);

        -- 🎰 [AUTO-ASSIGN] Handled round-robin (WITH NOTIFICATION)
        WHEN 'auto_assign_leads' THEN
            ...
            BEGIN
                ...
                IF v_num_staff > 0 AND array_length(v_unassigned_ids, 1) > 0 THEN
                    ...
                    -- PUSH NOTIFICATIONS 🔔
                    FOR i IN 1..v_num_staff LOOP
                        PERFORM public.fn_trigger_push_notification(v_staff_ids[i], 'New Leads Assigned 🎯', 'You received new automatically assigned leads.', '{"route":"/staff-dashboard"}'::jsonb);
                    END LOOP;
                END IF;
        ...
        -- 🧬 [REASSIGN] Reassign a single lead (WITH NOTIFICATION)
        WHEN 'reassign_lead' THEN
            v_staff_id := (p_payload->>'new_staff_id')::UUID;
            UPDATE public.profiles
            SET assigned_to = v_staff_id, updated_at = NOW()
            WHERE user_id = (p_payload->>'target_user_id')::UUID;
            
            -- PUSH NOTIFICATION 🔔
            PERFORM public.fn_trigger_push_notification(v_staff_id, 'Lead Reassigned 🎯', 'Admin has reassigned a specific lead to you.', '{"route":"/staff-dashboard"}'::jsonb);
            v_result := jsonb_build_object('status', 'success');
...
-- 2. 🔔 DATABASE TRIGGER: NOTIFY ADMINS ON CONVERSION
CREATE OR REPLACE FUNCTION public.fn_on_call_status_update_notify() RETURNS TRIGGER AS $$
DECLARE
    v_admin_user_id UUID;
    v_telecaller_name TEXT;
BEGIN
    -- Only trigger when call_status changes to 'converted' from something else
    IF NEW.call_status = 'converted' AND OLD.call_status IS DISTINCT FROM 'converted' THEN
        
        -- Get telecaller's name
        IF NEW.assigned_to IS NOT NULL THEN
            SELECT full_name INTO v_telecaller_name FROM public.profiles WHERE user_id = NEW.assigned_to LIMIT 1;
        END IF;

        -- Notify all admins
        FOR v_admin_user_id IN (SELECT user_id FROM public.profiles WHERE role = 'admin') LOOP
            PERFORM public.fn_trigger_push_notification(
                v_admin_user_id,
                'Lead Converted! 🎉',
                COALESCE(v_telecaller_name, 'A staff member') || ' successfully converted ' || NEW.full_name || '''s profile!',
                jsonb_build_object('type', 'conversion', 'route', '/admin-dashboard')
            );
        END LOOP;
...
-- Map the trigger accurately to public.profiles updates
DROP TRIGGER IF EXISTS tr_call_status_update ON public.profiles;
CREATE TRIGGER tr_call_status_update
AFTER UPDATE OF call_status ON public.profiles 
FOR EACH ROW EXECUTE FUNCTION public.fn_on_call_status_update_notify();
