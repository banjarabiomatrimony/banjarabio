...
/// Self Profile Screen - View, preview and edit own profile dashboard (Ultra-Premium Edition)
...
typedef MyProfileScreen = SelfProfileScreen;
...
class _SelfProfileScreenState extends ConsumerState<SelfProfileScreen>
  ...
  bool get _isSelfProfileTourActive {
    ...
    return cache.isGuestMode() &&
        !cache.isTourStageCompleted(TourStage.myProfileScreen.name);
  }
    ...
    try {
      ...
      await profileRes.fold(
        ...
        onFailure: (error) {
          AppLogger.error(
            'MyProfileScreen',
            'MyProfileScreen: Error loading profile: $error',
          );
          if (mounted) {
            setState(() {
              _errorMessage =
      ...
      AppLogger.error(
        'MyProfileScreen',
        'MyProfileScreen: Critical error loading profile: $e',
      );
      if (mounted) {
        setState(() {
          _errorMessage =
              AppLocalizations.of(context)?.criticalFailure(e.toString()) ??
              'Critical failure: $e';
          _isLoading = false;
        });
      }
  ...
  void _checkAndStartTour() {
    ...
    Future.delayed(const Duration(milliseconds: 800), () {
      ...
      tourService.startTour(
        ...
        onFinish: () {
          LocalCacheService().setTourStageCompleted(
            TourStage.myProfileScreen.name,
            true,
          );
        },
        onSkip: () {
  ...
  Widget build(BuildContext context) {
    ...
    try {
      ...
      AppLogger.error(
        'MyProfileScreen',
        'MyProfileScreen: Error getting theme: $e',
      );
      theme = ThemeData.light();
    }

    final liveTrustScore = ref
        .watch(trustScoreProvider)
        .maybeWhen(data: (score) => score, orElse: () => _trustScore);
                ...
                slivers: [
                    ...
                    actions: [
                      if (_profile != null)
                        _AnimatedEditProfileButton(
                          key: _isSelfProfileTourActive
                              ? TourKeys.profileEditKey
                              : null,
                          onTap: () {
                            Navigator.pushNamed(
                              context,
                  ...
                  SliverToBoxAdapter(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        // 👑 1. User Profile Identity & Completion Dashboard Card
                        _buildAnimatedItem(
                          index: 0,
                          child: _buildProfileDashboardCard(theme),
                        ),

                        // 🛡️ 2. Prominent Interactive Trust & Verification Card
                        if (_profile != null)
                          _buildAnimatedItem(
                            index: 1,
                            child: Padding(
                        ...
                        // 🌟 3. Animated Profile Section Tab Selector
                        _buildAnimatedItem(
                          index: 2,
                          child: ProfileTabSelectorWidget(
                            ...
                            margin: EdgeInsets.only(top: 0.2.h, bottom: 1.2.h),
                          ),
                        ),
  ...
  Widget _safeBuild(String label, Widget Function() builder) {
    try {
      ...
      AppLogger.error(
        'MyProfileScreen',
        'MyProfileScreen: Error building $label: $e',
      );
      return SizedBox(
        height: 6.h,
        child: Center(child: Text('Error: $label')),
      );
    }
  }
  ...
  Widget _buildProfileDashboardCard(ThemeData theme) {
    ...
    return Container(
      margin: EdgeInsets.only(top: 1.h, bottom: 1.2.h, left: 4.w, right: 4.w),
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // 1. Avatar and Identity Row (with Live Multi-Layer Pulse Ring)
          Row(
            children: [
              // Live Breathing Avatar
              GestureDetector(
                key: _isSelfProfileTourActive
                    ? TourKeys.profileManagePhotosKey
                    : null,
                onTap: () {
                  Navigator.pushNamed(
                    context,
                    AppRoutes.photoManagement,
                  ).then((_) => _loadProfile(forceRefresh: true));
                },
                        ...
                        InkWell(
                          onTap: () {
                            ...
                            Fluttertoast.showToast(
                              msg: 'Profile ID copied: ${_profile!.displayId}',
            ...
            children: [
              Text(
                ...
                style: TextStyle(
                  fontFamily: AppTypography.bodyFontFamily,
                  fontWeight: AppTypography.extraBold,
                  color: isComplete ? AppColors.categoryLocation : AppColors.slate500,
                  fontSize: AppTypography.labelSmall,
                ),
              ),
              Text(
...
class _AnimatedEditProfileButton extends StatefulWidget {
  ...
  const _AnimatedEditProfileButton({super.key, required this.onTap});
